...
class Flag:
    ...
    def unusual_screenTest(self, tickers=[], date=None, days=1):
        #
            ...
            for expiry_table in expiry['table_name']:
                ...
                try:
                    if len(tickers) > 0:
                        ...
                        request = "select * from {0} where symbol in ({1}) and date >= '{2}' and date <= '{3}' order by symbol date asc".format(expiry_table, ",".join(tickers), min_date, date)
                    else:
                        request = "select * from {0} where date >= '{1}' and date <= '{2}' order by symbol date asc".format(expiry_table, min_date, date)

                    option_data = pd.read_sql_query(request, con=connection_data)
                    ticker_count = option_data['symbol'].nunique()
                    date_count = option_data['date'].nunique()
                    print("{0} | Read Option Data | {1} | {2} Recs | {3} Tickers | {4} Days ".format(printid, expiry_table, len(option_data), ticker_count, date_count))
                ...
                try:
                    ...
                    chunks = [chunk[1] for chunk in option_data.groupby('symbol')]
                    pool = multiprocessing.Pool(processes=num_processes)
                    total_flags += pool.map(self.processFlagByTicker, chunks, chunksize=1)
                    pool.close()
                    pool.join()
    ...
    def processFlagByTicker(self, df_daily):
        ...
        printid = "{0} | Flag Screen | {1} : {2} | {3} Days".format(ticker_inp, min_date, max_date, total_days)
        ...
        try:
            ...
            print('{0} | Uploaded Flags | {1} Flags | {2}'.format(printid, len(temp_flags), ticker_end - ticker_start))
